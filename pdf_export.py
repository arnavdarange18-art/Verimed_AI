...
def generate_verification_report_pdf(
    ...
    pdf.set_xy(10, 6)
    pdf.cell(0, 10, "VeriMed AI -- Claim Verification Report", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 9)
    pdf.set_xy(10, 15)
    pdf.cell(0, 6, "AI-Generated, Evidence-Grounded Fact-Check", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(14)

    # ---- Claim text ----
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(*MUTED_RGB)
    pdf.cell(0, 6, "CLAIM SUBMITTED", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "I", 12)
    pdf.set_text_color(*TEXT_RGB)
    pdf.multi_cell(0, 7, f'"{claim_text}"')
    pdf.ln(3)
    ...
    pdf.cell(90, 12, f"  VERDICT: {display_verdict.upper()}", fill=True, new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)

    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(*MUTED_RGB)
    pdf.cell(0, 6, f"Trust Score: {confidence}%", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)

    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(*TEXT_RGB)
    pdf.multi_cell(0, 6, explanation or "No explanation available.")
    pdf.ln(3)
    ...
    if entities:
        ...
        pdf.cell(0, 7, "BioBERT NER Entities Detected", new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(*TEXT_RGB)
        entity_line = ", ".join(e.get("text", "") for e in entities)
        pdf.multi_cell(0, 6, entity_line)
        pdf.ln(2)

    # ---- Sources ----
    pdf.set_font("Helvetica", "B", 10)
    pdf.set_text_color(*SECONDARY_RGB)
    pdf.cell(0, 7, "Cited Sources", new_x="LMARGIN", new_y="NEXT")
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(*TEXT_RGB)
    pdf.multi_cell(0, 6, ", ".join(sources) if sources else "No sources returned.")
    pdf.ln(4)
    ...
    if method_comparison and method_comparison.get("methods"):
        ...
        pdf.set_text_color(*PRIMARY_RGB)
        pdf.cell(0, 8, "Detection Method Comparison", new_x="LMARGIN", new_y="NEXT")
        ...
        for m in method_comparison["methods"]:
            ...
            pdf.cell(0, 6, f"{m['name']}: {m['score']}/100 -- {m['label']}{winner_tag}", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("Helvetica", "", 9)
            pdf.set_text_color(*MUTED_RGB)
            pdf.multi_cell(0, 5, m.get("description", ""))
            pdf.ln(1)

        pdf.ln(2)
        pdf.set_font("Helvetica", "B", 10)
        pdf.set_text_color(*PRIMARY_RGB)
        pdf.cell(0, 6, "Conclusion", new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(*TEXT_RGB)
        pdf.multi_cell(0, 6, method_comparison.get("conclusion", ""))

    # ---- Footer ----
    pdf.set_y(-15)
    pdf.set_font("Helvetica", "I", 7)
    pdf.set_text_color(*MUTED_RGB)
    pdf.cell(0, 10, "Generated by VeriMed AI. Educational project -- always verify critical health decisions with a professional.", align="C")
